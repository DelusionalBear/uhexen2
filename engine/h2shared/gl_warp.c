...
void R_UpdateWarpTextures(void)
{
	...
	for (i = 0; i < cl.worldmodel->numtextures; i++)
	{
		...
		for (x = 0.0; x < 128.0; x = x2)
		{
			...
			{
				glTexCoord2f_fp(WARPCALC(x, y), WARPCALC(y, x));
				glVertex2f_fp(x, y);
				glTexCoord2f_fp(WARPCALC(x2, y), WARPCALC(y, x2));
				glVertex2f_fp(x2, y);
			}
			glEnd_fp();
		}
		...
		glCopyTexSubImage2D_fp(GL_TEXTURE_2D, 0, 0, 0, glx, gly + glheight - gl_warpimagesize, gl_warpimagesize, gl_warpimagesize);
